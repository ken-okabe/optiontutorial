(function()
	...
	liftCallback = function(desc, wrapped)
	{
        ...
        function(values, callback)
				{
					return f.apply(null, __slice.call(values)
						.concat([callback]));
        }
      ]);
			return withDescription.apply(null, [Bacon, desc, f].concat(__slice.call(args), [Bacon.combineAsArray(args)
				.flatMap(stream)]));
		});
	};
	...
	Bacon.combineWith = function()
	{
		var f, streams;
		f = arguments[0], streams = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
		return withDescription.apply(null, [Bacon, "combineWith", f].concat(__slice.call(streams), [Bacon.combineAsArray(streams)
			.map(function(values)
			{
	...
	Bacon.combineTemplate = function(template)
	{
		...
		return withDescription(Bacon, "combineTemplate", template, Bacon.combineAsArray(streams)
			.map(combinator));
	};
	...
	Observable = (function()
	{
		...
		{
			return withDescription(this, "fold", seed, f, this.scan(seed, f)
				.sampledBy(this.filter(false)
					.mapEnd()
					.toProperty()));
		};
		...
		Observable.prototype.log = function()
		{
			...
			{
				return typeof console !== "undefined" && console !== null ? typeof console.log === "function" ? console.log.apply(console, __slice.call(args)
					.concat([event.toString()])) : void 0 : void 0;
			});
			return this;
		};
		...
		Observable.prototype.combine = function(other, f)
		{
			...
			return withDescription(this, "combine", other, f, Bacon.combineAsArray(this, other)
				.map(function(values)
				{
		...
		{
			return withDescription(this, "awaiting", other, Bacon.groupSimultaneous(this, other)
				.map(function(_arg)
				{
	...
	EventStream = (function(_super)
	{
		...
		{
			return withDescription(this, "throttle", delay, this.bufferWithTime(delay)
				.map(function(values)
				{
		...
		{
			return withDescription(this, "sampledBy", sampler, combinator, this.toProperty()
		...
		EventStream.prototype.takeUntil = function(stopper)
		{
			var endMarker;
			endMarker = {};
			return withDescription(this, "takeUntil", stopper, Bacon.groupSimultaneous(this.mapEnd(endMarker), stopper.skipErrors())
		...
		{
			return withDescription(this, "startWith", seed, Bacon.once(seed)
				.concat(this));
		};
	...
	Property = (function(_super)
	{
		...
		function Property(desc, subscribe, handler)
		{
			...
			{
				this.subscribeInternal = new PropertyDispatcher(this, subscribe, handler)
					.subscribe;
			}
	...
	addPropertyInitValueToStream = function(property, stream)
	{
		...
		return justInitValue.concat(stream)
			.toProperty();
	};
				...
				if (current.isDefined && (_this.hasSubscribers() || ended))
				{
					...
					if (!ended && valId && dispatchingId && dispatchingId !== valId)
					{
						UpdateBarrier.whenDone(p, function()
						{
							...
							{
								return sink(initial(current.get()
									.value()));
							}
						});
					...
					else
					{
						...
						{
							return reply = sink(initial(current.get()
								.value()));
						}), []);
						return maybeSubSource();
					}
	...
	withDescription = function()
	{
		...
		return describe.apply(null, desc)
			.apply(obs);
	};
	...
	Bacon.update = function()
	{
		...
		return withDescription.apply(null, [Bacon, "update", initial].concat(__slice.call(patterns), [Bacon.when.apply(Bacon, patterns)
	...
	compositeUnsubscribe = function()
	{
		...
		return new CompositeUnsubscribe(ss)
			.unsubscribe;
	};
